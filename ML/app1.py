...
st.set_page_config(page_title="HackSparrow Consultancy")

st.write('<h1 style="font-family: cursive; font-size: 36px; color: #4CAF50; text-shadow: 2px 2px 4px rgba(0,0,0,0.4);">HackSparrow Way to Become a House Owner</h1>', unsafe_allow_html=True)

input1=st.text_input("Salary ",key="input1")
input2=st.text_input("Expense",key="input2")
input3=st.text_input("Home value",key="input3")
...
submit=st.button("provide plan")

## If ask button is clicked


st.write('<h1 style="color:goldenrod; font-size: 24px; text-align: center;">First of all be positive you are going to achieve a large milestone in the future and it\'s just a home... You will do it!</h1>', unsafe_allow_html=True)
if submit:
    
    response=get_response(input)
    st.subheader("Here it goes")
    st.write('<p style="color:red; font-size:16px; font-weight:bold; padding:10px;">***Disclaimer: The information provided here is for educational and information purposes only. It should not be considered as financial advice. Before making any investment decisions, please consult with a qualified financial advisor.</p>', unsafe_allow_html=True)
    st.write(response)
